...
class TodoBot:
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not update.effective_user or not update.effective_chat:
            return
        # Використовуємо chat_id для групових чатів, user_id для приватних
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        logger.info(f"Повідомлення від {update.effective_user.first_name}: '{text}' в чаті {chat_type} (ID: {user_id})")
    ...
    async def _send_message(self, update: Update, text: str, keyboard=None):
        ...
        if not update.effective_chat:
            logger.error("effective_chat is None")
            return
        try:
...
def main():
    ...
    application.add_handler(CommandHandler("list", bot.help_command))  # Додаткова команда для груп
